...
По умолчанию заявки обрабатываются по одной. Чтобы перекрывать ожидание ответа провайдера через fiber-исполнитель:

```bash
ruby bin/route --concurrent
```

Тот же режим можно включить в `config/routing_policy.yml` (`concurrency.enabled: true`). Флаг `--concurrent` имеет приоритет над файлом.
